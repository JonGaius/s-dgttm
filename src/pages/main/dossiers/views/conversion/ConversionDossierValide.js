...
const ConversionDossierValide = ({title}) => {
    ...
    return (   
        ...
        <FileDarianne actuel={'Dossiers Conversion validés'}>
            <OldFileDariane link={'/'}>
                Accueil
            </OldFileDariane>
            ...
            <div className='sigepec-module-dossier-list'>
                <div className='sigepec-module-dossier-list__head has--flex_between is--large'>
                    <h2>Liste des dossiers de conversion validés</h2>
                        ...
                        </div>
                        {
                            tableElement.map((element) => (
                                <div className='sigepec-table__body' key={element.id}>
                                    ...
                                    <div className='sigepec-table__column is--lilmed'>
                                    <Link to={`/gestion-des-dossiers/voir/conversions/${element.slug}`}><span>{element.dossier}</span></Link>  
                                    </div>
                                    <div className='sigepec-table__column is--auto'>
                                    <Link to={`/gestion-des-dossiers/voir/conversions/${element.slug}`}>{element.nom + " " + element.prenom} </Link>
                                    </div>
                                    <div className='sigepec-table__column is--lilmed'>
                                        {
                                            (element.statut === 0) ? 
                                    ...
                                    <div className='sigepec-table__column is--little'>
                                        ...
                                        <div className='sigepec-table__option' id={element.slug}>
                                            <Link to={`/gestion-des-dossiers/modifier/conversion/${element.slug}`}>
                                                Modifier le dossier
                                            </Link>
                                            <button type='button'>
                                                Supprimer
...
export default ConversionDossierValide;